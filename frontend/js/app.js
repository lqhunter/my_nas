  ...
  grid.innerHTML = items
    .map((item) => {
      ...
      const thumbHtml = isDir
        ...
        : ft === "video"
        ? `<img src="${getThumbnailUrl(item.path)}" alt="${item.name}" loading="lazy" onerror="this.style.display='none'"/><div class="play-badge"><svg viewBox="0 0 24 24" width="48" height="48" fill="#fff"><polygon points="6 4 20 12 6 20 6 4"/></svg></div>`
        : ft === "image"
